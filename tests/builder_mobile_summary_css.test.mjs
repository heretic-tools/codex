...
test("mobile roster summary is a bottom safe-area bar", () => {
  ...
  assert.ok(mobileLayer.includes("padding-bottom: calc(176px + env(safe-area-inset-bottom));"));
...
test("mobile Builder action controls keep 44px touch targets", () => {
  ...
  assert.ok(mobileLayer.includes(".roster-actions-panel"));
  assert.ok(mobileLayer.includes("right: calc(100% + 4px);"));
