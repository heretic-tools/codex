...
- 2026-07-06: Added a Warlord candidate-status guard to
  `builder_roster_warlord_actions.js`. The mutation layer now derives current
...
- 2026-07-06: Added an attachment-pair rule guard to
  `builder_roster_attachment_add_actions.js`. The add action now derives
  current unit summaries when explicit summaries are omitted, while the Builder
  UI can still pass precomputed summaries. Invalid bodyguard pairs cannot be
  written by direct action calls.
