...
скруглениями, mono-числами и приглушённым системным accent. Пользовательское
уточнение поверх исследования: это не dark-only продукт; светлый/тёмный режим
нужен на раннем этапе, поэтому themeable tokens входят в P0, а light theme
является полноценным override тех же tokens.

Обновление от 2026-07-08: текущая dark palette признана слишком резкой.
Дальнейший дизайн должен идти через спокойные shared tokens, без hot-orange /
pure-black / hardcoded bright-blue возвратов. Изображения фракций и юнитов
используются как приглушённый background/context layer в шапках, кнопках
ростеров и кнопках юнитов, а не как отдельная декоративная плашка, если
конкретный экран позволяет сохранить читаемость текста.
