...
3. Удаление юнита/модели — мгновенное действие с undo-toast с ограничением по времени, вместо confirm-диалога на каждое удаление. **Частично сделано:** удаление юнита, детача, attached unit, участника attached unit и всего ростера сохраняет предыдущий snapshot и показывает Undo; смена composition и Reset Wargear на unit detail тоже откатываются через Undo, потому что они могут сбрасывать модели/варгир. Модельные/composition-specific controls ещё требуют отдельного UX-прохода.
