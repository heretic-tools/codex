...
1. Итоговые очки ростера — sticky и itemized на всех вьюпортах (не только видны по скроллу вверх), с safe-area отступом на мобильном (зависит от Фазы 3/4). **Частично сделано:** roster detail и unit detail получили мобильную bottom sticky-сводку Points/DP/Units над footer с safe-area отступом; roster detail получил action shortcuts к Issues/Add Detachment/Add Unit/Attach controls; mobile sticky-сводка сжата до компактной двухрядной компоновки, а unit detail получил меньший нижний reserve; shortcuts центрируют первый рабочий control секции, чтобы форма не оказывалась под bottom overlay; sticky `+ Detach` и `+ Unit` теперь нажимают primary Add внутри соответствующей секции через существующую undo-aware логику. `Attach` остаётся навигацией к раскрываемой форме и открывает disclosure, когда attachment controls доступны. Unit-detail validation/focus jumps тоже центрируют первый рабочий control и раскрывают disclosure при наличии.
