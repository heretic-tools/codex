...
| P0/P1 | Builder в новом стиле | Частично сделано | Список ростеров, создание ростера, базовый roster detail и validation surfaces уже переведены на новый shell; roster list получил detachment badges. Validation messages получили компактные счетчики и severity-маркеры, Attached Units получили видимые подписи, roster detail и unit detail получили мобильную sticky-сводку Points/DP/Units, удаление юнита и ростера получило undo-toast. | Довести unit detail, wargear/upgrades, модельные/attachment/detachment удаления и оставшиеся mobile-first сценарии до полноценного UX в новой системе. |
...
3. Удаление юнита/модели — мгновенное действие с undo-toast с ограничением по времени, вместо confirm-диалога на каждое удаление. **Частично сделано:** удаление юнита на roster detail сохраняет предыдущий roster snapshot и показывает Undo; Delete Roster восстанавливает удалённый документ через Undo без browser-confirm; модельные/attachment/detachment удаления ещё требуют отдельного UX-прохода.
