...
1. Итоговые очки ростера — sticky и itemized на всех вьюпортах (не только видны по скроллу вверх), с safe-area отступом на мобильном (зависит от Фазы 3/4). **Частично сделано:** roster detail и unit detail получили мобильную bottom sticky-сводку Points/DP/Units над footer с safe-area отступом; roster detail получил action shortcuts к Issues/Add Detachment/Add Unit/Attach controls; mobile sticky-сводка сжата до компактной двухрядной компоновки, а unit detail получил меньший нижний reserve. Прямое добавление из sticky bar ещё не закрыто.
