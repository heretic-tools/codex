...
| P0/P1 | Builder в новом стиле | Частично сделано | Список ростеров, создание ростера, базовый roster detail и validation surfaces уже переведены на новый shell; roster list получил detachment badges. Validation messages получили компактные счетчики и severity-маркеры, Attached Units получили видимые подписи, roster detail и unit detail получили мобильную sticky-сводку Points/DP/Units, добавления/удаления roster unit/detachment/attachment и destructive unit-detail изменения composition/reset/wargear-count/upgrades получили undo-toast, roster-level и unit-level Warlord selectors используют тот же undo flow, unit row получил touch swipe-delete, unit detail Wargear скрывает пустые scopes без validation messages, roster editor больше не рендерит промежуточную `builder-section` вокруг Detachments/Units/Attached Units. | Довести unit detail, модельные/composition-specific controls и оставшиеся mobile-first сценарии до полноценного UX в новой системе. |
...
3. Удаление юнита/модели — мгновенное действие с undo-toast с ограничением по времени, вместо confirm-диалога на каждое удаление. **Частично сделано:** добавление и удаление юнита/детача/attached unit, удаление участника attached unit и всего ростера сохраняет предыдущий snapshot и показывает Undo; roster-level Warlord picker, смена composition, Reset Wargear, прямое изменение wargear count, Warlord, Allegiance и Enhancement selectors на unit detail тоже откатываются через Undo. Модельные/composition-specific controls ещё требуют отдельного UX-прохода.
