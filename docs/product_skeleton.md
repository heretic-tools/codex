...
- Целевой визуальный язык — dark-first Grimdark Tactical Mono: плотный
  tactical UI, плоские 1px borders, минимальные скругления, mono-числа,
  янтарный акцент.
- Подход к темам — dark-first, не dark-only: dark theme является дефолтом и
  основным tabletop режимом; light theme доступна на раннем этапе как
  полноценный override тех же tokens, а не отдельная вторая верстка.
...
- Attached Units не должны выглядеть как обязательная куча полей: пустое
  ...
  только когда есть допустимые пары и раскрываются через явное действие
  `Add attached unit`.
