...
function validateWarlord(roster, detachments, units, messages) {
  ...
  if (mandatoryWarlordId) {
    ...
    if (!mandatoryPresent) {
      messages.push({ level: "error", text: `${faction.name || "Faction"} requires ${mandatoryWarlord?.name || "a required model"} in your army.` });
    } else if (!warlordIds.length) {
      messages.push({ level: "error", text: `${faction.name || "Faction"} requires ${mandatoryWarlord?.name || "a required model"} as Warlord.` });
    }
  }
  if (!warlordIds.length) {
    if (!mandatoryWarlordId) {
      messages.push({ level: "error", text: "Pick one Warlord." });
    }
    return;
  }
  if (warlordIds.length > 1) {
    messages.push({ level: "error", text: "Roster has more than one Warlord." });
  }
  ...
  if (supremeCommanders.length && !supremeCommanders.some((item) => item.miniatureId === selectedWarlordId)) {
    messages.push({ level: "error", text: "One of the Supreme Commander units must be your Warlord." });
  }
  ...
  if (mandatoryRows.length && !mandatoryRows.some((row) => row.miniatureId === selectedWarlordId)) {
    ...
    messages.push({ level: "error", text: `${mandatoryRows[0].detachmentName} requires one of these Warlords: ${names}.` });
  }
  ...
  if (selectedUnit && selectedMiniature && !canBeWarlord(selectedMiniature, selectedUnit, roster, detachmentIds, warlordIds)) {
    messages.push({ level: "error", text: "Selected Warlord is not eligible." });
  }
}
